...
## Live mail setup and checkout review — September 9, 2026

- `25ceecf`: added `npm run mail:check`, shared startup validation and a concrete Resend setup guide. A managed receiving subdomain avoids buying a domain. Partial mail configuration no longer advertises an unusable route; conflicting listener ports fail before database startup. TypeScript and 33 focused tests passed. The real local check correctly reported missing setup without contacting a provider or printing secrets. No account, forwarding rule or public ingress was created.
- Extension 0.3.1 adds **Read pending checkout** in My planner. The strict browser-only protocol shows recognized registration rows, units and grade options; checks the entire coursebin status list against the chosen plan; and rejects unfamiliar/mixed markup. It shares the coursebin operation lock, expires old reviews, rejects late planner results and accepts no submit/endpoint command. Prompts describe the actual read-only capability.
- `npm run check` passed **338 tests across 31 files**, TypeScript and both extension bundles. The signed-out real Codex companion smoke also passed. New fixtures cover complete/extra/missing selections, existing registrations, pending drops, invalid form structure, no token reads or clicks, changed grade fields, authentication failure, unauthorized senders, contention/timeouts and mounted review UI.
- The production parser and built content adapter both succeeded on authenticated Fall 2026 Checkout in the separate diagnostic browser, including the adapter's fixed CourseBin GET and status parsing. Only sanitized structural outcomes were returned. Fresh navigation resolved an earlier failed read via authenticated Terms and normal semester reselection; sign-in expiry was not established. No enrollment or coursebin change occurred. A 500-pixel web preview showed the review UI with no overflow and browser-only actions correctly disabled.
- **Still unfinished:** live email activation (needs the developer's Resend receiving domain, locally configured credentials and approved HTTPS ingress); concrete enrollment confirmation/executor and authoritative result reconciliation (needs the student's intended pilot sections, followed by separately approved real submission). The current release cannot automatically register. See [checkout review](checkout-review.md) and [mail setup](mail-setup.md).
