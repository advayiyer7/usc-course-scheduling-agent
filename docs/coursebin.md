...
3. Click **Edit in planner** on one draft, then **Add to coursebin** at the top of **My planner**. Opening a draft or switching tabs preserves both alternatives; manual course selection is unnecessary. The action reviews the current planner selection. If data is stale, use **Refresh data and recheck**; it waits for fresh data without substituting or removing selected sections.
...
6. Follow section results in the planner and chat: **added**, **already present**, **failed**, or **unconfirmed**. Review myCourseBin and complete checkout yourself. **Ask companion for alternatives** sends only the selected sections' structured results; any replacement requires its own draft confirmation.
...
The session-only journal contains the selected proposal's outcome IDs and pending section, not unrelated bin entries or raw page data. It is not sent to the backend. Closing/reopening the side panel restores it in the planner and chat. The active-run Stop control remains above both tabs. A restarted service worker marks pending work interrupted/unconfirmed and never resumes automatically. An unfamiliar response or timeout remains unconfirmed; a bounded read can reconcile late presence. A new student-triggered review re-reads the bin before another attempt.
