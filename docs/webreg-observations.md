...
The installed extension was subsequently reloaded from integration commit `482d0cb` (feature commit `8f7c7cb`), followed by a WebReg page refresh. Refreshing the selected course records preserved the exact planner selection. The planner's **Add to coursebin** preparation action successfully opened the semester-specific review with the same sections; the previous `UI_CHANGED` error was absent. Required-component/link rules remained unresolved, so final confirmation stayed disabled. No confirmation, add, registration or checkout action was invoked. This verifies the real installed extension's read-only bridge and bin parser, not its live mutation path. The code change passed 246 tests, TypeScript checks, both builds and integration CI.
