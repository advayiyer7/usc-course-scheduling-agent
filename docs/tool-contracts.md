...
Fall 2026 EE109, CSCI426 and SSCI165 have narrowly verified unlinked component profiles. A `component_rules` finding may include `evidence` with the policy version, review date and official sources. Passing requires complete source inventory and explicit empty registration notes; unsupported courses, links, modes, sessions or missing provenance remain unknown. Normalized courses expose the full `section_ids` inventory and optional course/term `registration_notes`; sections expose optional `registration_notes`. Missing fields in older snapshots are not empty instructions. See [component rules and limits](component-validation.md).
