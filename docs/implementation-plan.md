...
The user has tested authenticated chat. Remaining dependencies are source-backed GE-category discovery, verified component/date semantics, and the WebReg adapter/action UI being built in a separate task. The current **Load into planner** button is implemented; **Add to coursebin** is not on main. This acceptance target does not claim those dependencies are complete or authorize changes to a real student's bin without their selection of a specific schedule.
