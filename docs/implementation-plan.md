...
2. It retrieves the relevant sections, accounts for required components and stated preferences, and presents up to two validated alternatives in chat when available. If fewer exist or facts are unresolved, explain that rather than inventing alternatives or relaxing hard constraints silently.
3. Each coursebin action is attached to a specific reviewed selection. Per the latest UI request, the student opens a draft with **Edit in planner**, then clicks **Add to coursebin** on **My planner**. Both alternatives survive this handoff and navigation; manual course selection is unnecessary.
...
The user has tested authenticated chat. The bounded WebReg adapter, planner **Add to coursebin** confirmation and structured outcome UI are now implemented and fixture-tested with **Edit in planner** for draft handoff. Remaining dependencies are source-backed GE-category discovery, verified component/date semantics, empty-bin evidence and a student-authorized live addition test. The Fall 2026 EE109, CSCI426 and SSCI165 component profiles are implemented with complete-source guards; unsupported configurations and stale selected records still block. See [component coverage](component-validation.md). See [coursebin evidence and gates](coursebin.md). This does not authorize changes to a real student's bin without their selection and explicit confirmation of a specific schedule.

## Authorized validation and clearance milestone — September 6, 2026

Implemented a shared validation review with explicit date evidence, component uncertainty, snapshot seat counts and independent eligibility. Clearance links are curated official USC instructions scoped by teaching department, course level, term and audience; unknown routes remain explicit. Cards show the results, and manual planner edits automatically revalidate. [Coverage and research](d-clearance.md) distinguish department-specific guidance from directory fallbacks. Required component semantics remain an open research/implementation task; this milestone does not make every schedule coursebin-ready.
