...
if (!dir)
  throw new Error(
    "Usage: npm run import:snapshot -- /path/to/extracted-snapshot [term]",
  );
const term = termCode.parse(Number(process.argv[3] ?? 20263));
const programs: unknown = JSON.parse(
  await readFile(join(dir, "programs.json"), "utf8"),
);
const responses: SourceResponse[] = [];
for (const file of (await readdir(join(dir, "programs"))).filter((f) =>
  f.endsWith(".json"),
)) {
  const p = join(dir, "programs", file),
  ...
  responses.push({
    school,
    program,
    checked_at: (await stat(p)).mtime.toISOString(),
...
try {
  console.log(
    JSON.stringify(await store.publish(term, programs, responses), null, 2),
  );
} finally {
